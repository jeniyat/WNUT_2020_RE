...
class ProtoFile:
    ...
    def get_token_idx(self, tag):
        ...
        while start < tag.start:
            ...
            if s_idx + 1 == len(tokens1d) or sent_idx+1==len(self.lines):
                s_idx += 1
                break

            start = offset + html.unescape(self.lines[sent_idx + 1]).find(tokens1d[s_idx + 1].word,start - offset + len(tokens1d[s_idx].word))
        ...
        e_idx = s_idx
        while end < tag.end:
            if e_idx + 1 == len(tokens1d) or sent_idx+1==len(self.lines):
                e_idx += 1
                break
            try:
                end = offset + html.unescape(self.lines[sent_idx + 1]).find(tokens1d[e_idx + 1].word, end - offset + len(tokens1d[e_idx].word))
    ...
    def __gen_relations(self):
        ...
        for link in self.links:
            ...
            if sent_idx1 == sent_idx2:  # verify that both arg1 and arg2 have the same sent idx
                ...
                # if sent_idx1 == len(self.parse_trees):
                #     sent_idx1= sent_idx1-1
                try:
        ...
        for arg1, arg2 in tqdm(arg_perm, desc="arg_perm " + self.protocol_name, disable=True):
...
if __name__ == '__main__':
    ...
    filename_base = "/home/jeniya/WLP-RE-LR-baseline/WLP-Parser/WLP-Dataset/train_full/"
    filenames = [x.replace(".ann","") for x in os.listdir(filename_base) if x.endswith(".ann")]
    # print(filenames)
    # pro = ProtoFile("WLP-Dataset/dev/protocol_0")
    for i in range(0, 700):
        # if i==46 or i==105 or i==112 or i==304 or i==474 or i==570: continue
        protocol_name = "protocol_"+str(i)
        if protocol_name in filenames:
